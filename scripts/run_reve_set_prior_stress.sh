...
OUTPUT=outputs/reve_set_prior_stress_audit_s7
"${GAUGEEG[@]}" prior-stress \
  --validation-predictions "${E9_LOGITS}" \
  --output-dir "${OUTPUT}"

echo "Completed CPU-only E10: ${OUTPUT}"
python -m json.tool "${OUTPUT}/prior_stress_summary.json"
